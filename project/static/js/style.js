// ===================================
// STYLE.JS - Glassmorphism Framework
// ===================================
...
function initializeParticles() {
    ...
    particles.forEach((particle) => {
        ...
        particle.addEventListener('animationiteration', function () {
            ...
            this.style.left = `calc(${this.style.left || getComputedStyle(this).left} + ${randomOffset}px)`;
        });
    });
}
...
function initializeFormEffects() {
    ...
    document.querySelectorAll('.form-control').forEach(input => {
        input.addEventListener('focus', function () {
            const container = this.closest('.form-floating, .input-group');
        ...
        input.addEventListener('input', function () {
            this.style.background = this.value.length > 0
    ...
    document.querySelectorAll('.btn-primary-glass, .btn-secondary-glass, .btn-login, .btn-signup').forEach(btn => {
...
function initializeAnimations() {
    ...
    document.querySelectorAll('.glass-card-small, .content-area > *').forEach(el => {
        observer.observe(el);
    });
}
...
function checkPasswordStrength(passwordId = 'password', strengthBarId = 'strengthBar') {
...
function checkPasswordMatch(passwordId = 'password', confirmPasswordId = 'confirmPassword', matchIndicatorId = 'passwordMatch') {
    const password = document.getElementById(passwordId)?.value || '';
    const confirmPassword = document.getElementById(confirmPasswordId)?.value || '';
    const matchIndicator = document.getElementById(matchIndicatorId);
...
function hideLoading(buttonElement) {
    ...
    if (originalText) buttonElement.innerHTML = originalText;
...
function validateForm(formElement) {
    ...
    inputs.forEach(input => {
        if (!input.value.trim()) {
            ...
            input.style.boxShadow = '0 0 0 0.25rem rgba(255, 71, 87, 0.25)';
            isValid = false;
